...
export function buildTeamBenchmarkTemplate() {
  return { schema_version: 2, profile: "V1_5_RELEASE", synthetic: false, methodology: { same_task: true, same_repository_commit: true, same_host: true, same_model: true, repetitions_per_mode: 3, minimum_task_cases: 30 }, cases: [{ id: "replace-me", repository_commit: null, host: null, model: null, runs: [] }], notes: "Record all four modes under the same conditions across at least 30 task cases. Empty templates are INSUFFICIENT_EVIDENCE." };
}

export function evaluateTeamBenchmark(fixture) {
  if (!fixture || ![1, 2].includes(fixture.schema_version) || !Array.isArray(fixture.cases) || !fixture.cases.length) throw new Error("team benchmark fixture is invalid");
  ...
  if (fixture.schema_version === 2 && normalized.length < (fixture.methodology?.minimum_task_cases ?? 30)) problems.push(`release benchmark requires at least ${fixture.methodology?.minimum_task_cases ?? 30} task cases`);
  if (problems.length) return { schema_version: fixture.schema_version, status: "INSUFFICIENT_EVIDENCE", synthetic: Boolean(fixture.synthetic), modes, case_count: normalized.length, sample_size: normalized.reduce((sum, item) => sum + item.runs.length, 0), problems, results: [], conclusion_allowed: false };
  ...
  return { schema_version: fixture.schema_version, status, synthetic: Boolean(fixture.synthetic), modes, case_count: normalized.length, sample_size: normalized.reduce((sum, item) => sum + item.runs.length, 0), problems, results, conclusion_allowed: status === "MEASURED" };
}
