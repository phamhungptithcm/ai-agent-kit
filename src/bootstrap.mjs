...
function managedFilePlan(files, { includeClaude, includeCodex }) {
  const entries = new Map();
  const add = (filePath, mode) => entries.set(filePath, { path: filePath, mode });
  ...
  for (const relPath of files.keys()) {
    ...
    if (!match) continue;
    if (includeCodex) add(`.agents/skills/${match[1]}/SKILL.md`, "generated-file");
    if (includeClaude) add(`.claude/skills/${match[1]}/SKILL.md`, "generated-file");
  }
  add(".gitignore", "managed-section");
  add(".ai-agent-kit/project.yaml", "generated-file");
  add(".ai-agent-kit/installation.json", "generated-file");
  add(".ai-agent-kit/output/merge-request-description.md", "generated-file");
  add(".ai-agent-kit/output/jira-update.md", "generated-file");
...
No Git operations were performed.`);
    ...
    const managedFiles = managedFilePlan(scaffoldFiles, { includeClaude, includeCodex });
