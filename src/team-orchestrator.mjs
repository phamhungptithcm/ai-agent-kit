...
export function startTeam(options) {
  ...
  team.dispatch_instructions = team.assignments.map((item) => ({ assignment_id: item.id, native_profile: profiles[item.id] ?? item.id, objective: item.objective, depends_on: item.depends_on, phase: item.phase, required: item.required, blocking: item.blocking, write_access: item.write_access, allowed_paths: item.allowed_paths, shared_context: `.ai-agent-kit/runtime/team-contexts/${team.task_id}.json`, shared_context_trust: "UNTRUSTED_DATA", brief_hash: briefHash(context), expected_output_schema: "team-result-v1", token_budget: Math.floor(team.budgets.token_budget / team.assignments.length), timeout_seconds: team.budgets.timeout_seconds, max_attempts: item.max_attempts, required_handoff: true, forbidden_actions: ["commit", "push", "deploy", "publish", "release", "external-account-mutation"] }));
