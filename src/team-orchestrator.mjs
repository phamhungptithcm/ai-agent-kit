...
import { inspectTeamRegistry, registerRepositoryTask } from "./team-registry.mjs";
import { inspectIntegrationQueue } from "./team-integration.mjs";
...
export function startTeam(options) {
  ...
  if (team.planning?.planning_hash !== currentPlanningHash) team = planTeam({ target: root, id: options.id, goal, paths, risk: team.planning?.risk, shape: team.planning?.requested_shape, controlPlane: team.control_plane?.enabled, maxAgents: team.budgets.max_agents, tokenBudget: team.budgets.token_budget, timeoutSeconds: team.budgets.timeout_seconds, maxActions: team.budgets.max_actions, maxConcurrency: team.budgets.max_concurrency, now: options.now });
  ...
  if (team.control_plane?.enabled) {
    if (!options.identity) throw new Error("repository control-plane start requires an authenticated identity");
    const registry = inspectTeamRegistry({ target: root, now: options.now });
    const registered = registerRepositoryTask({ target: root, taskId: team.task_id, goalHash: team.goal_hash, parentCommit: team.control_plane.parent_commit, identity: options.identity, identitySecret: options.identitySecret, resolveIdentityKey: options.resolveIdentityKey, expectedRevision: registry.revision, now: options.now });
    team.control_plane.status = "ACTIVE"; team.control_plane.repository_id = registry.repository_id; team.control_plane.registry_revision = registered.revision;
  }
  ...
  team.dispatch_instructions = team.assignments.map((item) => ({ assignment_id: item.id, native_profile: profiles[item.id] ?? item.id, objective: item.objective, depends_on: item.depends_on, phase: item.phase, required: item.required, blocking: item.blocking, write_access: item.write_access, allowed_paths: item.allowed_paths, shared_context: `.ai-agent-kit/runtime/team-contexts/${team.task_id}.json`, shared_context_trust: "UNTRUSTED_DATA", brief_hash: briefHash(context), skill_route_hash: task.skill_routing?.config_hash ?? null, routed_skill: task.skill_routing?.skill ?? null, context_pack_hash: task.execution_context?.content_hash ?? null, expected_output_schema: "team-result-v1", token_budget: Math.floor(team.budgets.token_budget / team.assignments.length), timeout_seconds: team.budgets.timeout_seconds, max_attempts: item.max_attempts, required_handoff: true, repository_control_plane: team.control_plane?.enabled ? { repository_id: team.control_plane.repository_id, parent_commit: team.control_plane.parent_commit, fencing_required: true, isolated_workspace_required: item.write_access } : null, forbidden_actions: ["commit", "push", "merge", "deploy", "publish", "tag", "release", "repository-settings", "external-account-mutation"] }));
...
export function reportTeam(options) {
  ...
  if (team.control_plane?.enabled) {
    const registry = inspectTeamRegistry(options); const queue = inspectIntegrationQueue(options);
    const activeClaims = registry.claims.filter((item) => item.task_id === team.task_id && item.status === "ACTIVE");
    const writerIds = team.assignments.filter((item) => item.write_access && item.status === "COMPLETED").map((item) => item.id);
    const admitted = queue.packages.filter((item) => item.task_id === team.task_id && item.state === "ADMITTED");
    if (activeClaims.length) blockers.push(`${activeClaims.length} repository claims remain active`);
    for (const writerId of writerIds) if (!admitted.some((item) => item.assignment_id === writerId)) blockers.push(`writer ${writerId} has no admitted integration package`);
    if (team.assignments.some((item) => item.write_access && item.status === "COMPLETED" && (!item.execution?.repository_claim_id || !item.execution?.fencing_token || !item.execution?.workspace?.snapshot_hash))) blockers.push("completed writer is missing repository claim, fence, or workspace evidence");
    controlPlane = { status: blockers.length ? "NOT_READY" : "READY", repository_id: registry.repository_id, registry_revision: registry.revision, active_claims: activeClaims.length, admitted_packages: admitted.map((item) => item.package_id), queue_revision: queue.revision };
  }
  return { schema_version: 2, task_id: team.task_id, team_type: team.team_type, planning: team.planning, execution_mode: team.execution_mode, adapter_capabilities: team.adapter_capabilities, control_plane: controlPlane, run: team.run, status: blockers.length ? "NOT_READY" : optionalFailures.length || optionalFindings.length ? "DEGRADED" : "READY", completed_assignments: completed.length, total_assignments: team.assignments.length, optional_failures: optionalFailures.map((item) => ({ assignment_id: item.id, status: item.status })), optional_finding_count: optionalFindings.length, review_independence: reviewer && !reviewer.write_access ? "VERIFIED" : "REJECTED", findings, context: { status: context.status, revision: context.revision, knowledge_revision: context.knowledge_revision, handoff_count: context.handoff_count, open_conflicts: context.open_conflicts, context_hash: context.context_hash }, usage, budgets: team.budgets, evidence_hashes: [...new Set(completed.flatMap((item) => item.evidence_hashes))], blockers, team_hash: team.team_hash };
}
